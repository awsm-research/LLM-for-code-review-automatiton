...
def my_bleu_fromstr(predictions, golds, rmstop=True):
    predictions = [" ".join(nltk.wordpunct_tokenize(predictions[i])) for i in range(len(predictions))]
    golds = [" ".join(nltk.wordpunct_tokenize(g)) for g in golds]
    if rmstop:
        pypath = os.path.dirname(os.path.realpath(__file__))
    ...
    golds = [str(i) + "\t" + gold.replace("\t", " ") for (i, gold) in enumerate(golds)]
    goldMap, predictionMap = computeMaps(predictions, golds)
    bleu = my_bleuFromMaps(goldMap, predictionMap)
    # bleus = []
    # for pred, gold in zip(predictions, golds):
    #     gold = splitPuncts(gold.strip().lower())
    #     pred = splitPuncts(pred.strip().lower())
    #     bleus.append(bleu(gold,pred)[0] * 100.0)
    return bleu
...
def my_bleuFromMaps(m1, m2):
    scores = []
    for key in m1:
        if key in m2:
            bl = bleu(m1[key], m2[key][0])
            scores.append(bl[0]* 100.0)
    return scores
